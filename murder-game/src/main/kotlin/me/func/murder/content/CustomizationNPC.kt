...
import me.func.murder.donate.impl.MoneyKit
import me.func.murder.donate.impl.NameTag
import me.func.murder.donate.impl.StarterPack
...
class CustomizationNPC(private val game: MurderGame) {
    ...
    private val menu = ControlledInventory.builder()
        ...
        .provider(object : InventoryProvider {
            override fun init(player: Player, contents: InventoryContents) {
                ...
                contents.add('W', ClickableItem.of(item {
                    ...
                    subInventory(player, 6) { _: Player, currentContent: InventoryContents ->
                        ...
                        pasteItems(user, false, currentContent, Mask.values().asIterable()) {
                            ...
                            user.stat.mask.setMask(user) // todo
                        }
                    }
                })
                ...
                contents.add('O', ClickableItem.of(LootboxUnit.icon) {
                ...
                contents.add('D', ClickableItem.of(StarterPack.icon) {
                    donateMenu(player, StarterPack, true)
                })

                val countHaveAchievement =
                    Achievement.values().count { it.predicate(user) && !stat.achievement.contains(it) } // todo
                            ...
                            .forEach { achievement ->
                                ...
                                currentContent.add('I', ClickableItem.of(item {
                                    ...
                                    if (playerHas) {
                                        ...
                                        nbt("other", "new_booster_1")
                                        if (canGet && !playerHas) { // артем што это такое)))
    ...
    init {
        // Создание NPC
        val npcLabel = game.map.getLabel("guide")
        ...
        game.context.every(5) {
            if (game.status == RealmStatus.GAME_STARTED_RESTRICTED)
                return@every
            game.particleHelper.happyVillager(npcLabel)
        }
    ...
    fun pasteItems(
        ...
        item.forEach { currentItem ->
            content.add('I', ClickableItem.of(DonateHelper.modifiedItem(user, currentItem)) {
                if (user.stat.donate.contains(currentItem)) { // todo
                    fill(currentItem)
        ...
        subInventory(player, 1) { _, contents: InventoryContents ->
            ...
            contents.add('G', ClickableItem.of(accessItem) {
                ...
                if (realMoney)
                    ...
                    if (user.stat.donate.contains(donatePosition)) { // todo
    ...
    private fun buy(user: User, money: Int, desc: String, accept: (User) -> Unit) {
